...
namespace BMS {
	...
	void BMSFile::CompileNoteData() {
		...
		/*std::vector<BMSEvent> eventLists;
		
		for (auto& [measure, events] : m_events) {
			std::vector<BMSEvent> perMeasureEventLists;

			for (auto& data : events) {
				switch (data.Channel) {
					case 2: {
						BMSEvent ev = {};
						ev.Channel = data.Channel;
						ev.Measure = measure;
						ev.Value = std::atof(mergeVector(data.Params).c_str());
						ev.Position = 0;

						perMeasureEventLists.push_back(ev);
						break;
					}

					case 3: {
						for (int i = 0; i < data.Params.size(); i++) {
							auto& msg = data.Params[i];
							if (msg == "00") continue;

							BMSEvent ev = {};
							ev.Channel = data.Channel;
							ev.Measure = measure;
							ev.Position = static_cast<float>(i) / data.Params.size();

							try {
								ev.Value = std::stoi(msg, nullptr, 16);
							}
							catch (std::invalid_argument&) {
								::printf("[BMS] [ERROR] Failed to parse BPM, undefined behavior may occured!");
								continue;
					...
					case 9: {
						for (int i = 0; i < data.Params.size(); i++) {
							...
							if (m_stops.find(msg) != m_stops.end()) {
								BMSEvent ev = {};
								ev.Channel = data.Channel;
								ev.Measure = measure;
								ev.Position = static_cast<float>(i) / data.Params.size();
								ev.Value = m_stops[msg];
			...
			eventLists.insert(eventLists.end(), perMeasureEventLists.begin(), perMeasureEventLists.end());
		}*/
