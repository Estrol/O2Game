...
#include "SkinManager.h" // wtf recursive
...
void GetLuaState(ScriptState &state, sol::table &table)
{
    if (table["type"].get_type() != sol::type::number) {
        throw Exceptions::EstException("Script didn't return the expected value");
    }

    state.type = table["type"];

    if (table["init"].get_type() != sol::type::function) {
        throw Exceptions::EstException("Script did not have init function");
    }
...
void LuaScripting::TryLoadGroup(SkinGroup group)
{
    ...
    if (!std::filesystem::exists(fullPath)) {
        throw Exceptions::EstException("Missing file: %s", fullPath.string());
    }
    ...
    if (state.state.lua_state() == nullptr) {
        throw Exceptions::EstException("Failed to create lua state");
    }
...
std::vector<NumericValue> LuaScripting::GetNumeric(SkinGroup group, std::string key)
{
    try {
        if (m_states[group].init.lua_state() == NULL) {
            throw Exceptions::EstException("Lua state is null");
        }
        ...
        throw Exceptions::EstException(err.what());
    }
}
...
std::vector<NumericValue> LuaScripting::Arena_GetNumeric(std::string key)
{
    ...
    try {
        ...
        if (sol::type::table != result_table.get_type()) {
            throw Exceptions::EstException("expected returned function is table!");
        }

        if (sol::type::table != result_table[SkinDataType::Numeric].get_type()) {
            throw Exceptions::EstException("The table missing DataType.Numeric");
        }

        sol::table key_array = result_table[SkinDataType::Numeric][key];
        if (sol::type::table != key_array.get_type()) {
            throw Exceptions::EstException("The table has no Key: %s", key.c_str());
        }

        std::vector<NumericValue> result;
        for (auto &value : key_array) {
            if (value.second.get_type() != sol::type::table) {
                throw Exceptions::EstException("Array key is not table");
            }
            ...
            if (value_table.size() < 5) {
                throw Exceptions::EstException("Array key table size is less than 7");
            }

            auto x = value_table[1];
            if (x.get_type() != sol::type::number) {
                throw Exceptions::EstException("Numeric::X is not a number, Key: %s", key.c_str());
            }

            auto y = value_table[2];
            if (y.get_type() != sol::type::number) {
                throw Exceptions::EstException("Numeric::Y is not a number, Key: %s", key.c_str());
            }

            auto maxDigit = value_table[3];
            if (maxDigit.get_type() != sol::type::number) {
                throw Exceptions::EstException("Numeric::MaxDigit is not a number, Key: %s", key.c_str());
            }
            ...
            if (direct.get_type() == sol::type::string) {
                ...
                throw Exceptions::EstException("Numeric::Direction is not either LEFT, MID, RIGHT or -1, 0, 1, Key: %s", key.c_str());
            }

            auto fillWithZero = value_table[5];
            if (fillWithZero.get_type() != sol::type::number && fillWithZero.get_type() != sol::type::boolean) {
                throw Exceptions::EstException("Numeric::FillWithZero is not number or boolean, Key: %s", key.c_str());
            }
...
std::vector<PositionValue> LuaScripting::Arena_GetPosition(std::string key, int KeyCount)
{
    try {
        ...
        sol::table result_table = m_arena_states->init();
        if (sol::type::table != result_table.get_type()) {
            throw Exceptions::EstException("expected returned function is table, but got other than table");
        }

        if (sol::type::table != result_table[SkinDataType::Position].get_type()) {
            throw Exceptions::EstException("The table missing DataType.Position");
        }

        sol::table key_array = result_table[SkinDataType::Position][KeyCount];
        if (sol::type::table != key_array.get_type()) {
            throw Exceptions::EstException("The position table has no item for KeyCount: %d", KeyCount);
        }
        ...
        for (auto &value : key_array) {
            ...
            if (item1.get_type() != sol::type::number) {
                throw Exceptions::EstException("%s X is not a number", prefix.c_str());
            }

            if (item2.get_type() != sol::type::number) {
                throw Exceptions::EstException("%s Y is not a number", prefix.c_str());
            }

            if (item3.get_type() != sol::type::number) {
                throw Exceptions::EstException("%s AnchorPointX is not a number", prefix.c_str());
            }

            if (item4.get_type() != sol::type::number) {
                throw Exceptions::EstException("%s AnchorPointY is not a number", prefix.c_str());
            }
            ...
            if (item5.get_type() != sol::type::number) {
                throw Exceptions::EstException("%s RGB index Red is not a number", prefix.c_str());
            }

            if (item6.get_type() != sol::type::number) {
                throw Exceptions::EstException("%s RGB index Green is not a number", prefix.c_str());
            }

            if (item7.get_type() != sol::type::number) {
                throw Exceptions::EstException("%s RGB index Blue is not a number", prefix.c_str());
            }
...
std::vector<RectInfo> LuaScripting::Arena_GetRect(std::string key)
{
    try {
        ...
        if (sol::type::table != result_table[SkinDataType::Rect].get_type()) {
            throw Exceptions::EstException("The table missing DataType.Rect");
        }

        sol::table key_array = result_table[SkinDataType::Rect][key];
        if (sol::type::table != key_array.get_type()) {
            throw Exceptions::EstException("The table has no key: %s", key);
        }
        ...
        for (auto &value : key_array) {
            ...
            if (value_table.size() < 4) {
                throw Exceptions::EstException("Array key table size is less than 4");
            }
            ...
            if (item1.get_type() != sol::type::number) {
                throw Exceptions::EstException("Array key table item 1 is not number");
            }

            if (item2.get_type() != sol::type::number) {
                throw Exceptions::EstException("Array key table item 2 is not number");
            }

            if (item3.get_type() != sol::type::number) {
                throw Exceptions::EstException("Array key table item 3 is not number");
            }

            if (item4.get_type() != sol::type::number) {
                throw Exceptions::EstException("Array key table item 4 is not number");
            }
...
SpriteValue LuaScripting::Arena_GetSprite(std::string key)
{
    try {
        ...
        if (sol::type::table != result_table[SkinDataType::Sprite].get_type()) {
            throw Exceptions::EstException("The table missing DataType.Sprite");
        }
        ...
        if (key_array.size() < 6) {
            throw Exceptions::EstException("Expect table size is 6 at key: %s", key.c_str());
        }
        ...
        if (sol::type::number != item1.get_type()) {
            throw Exceptions::EstException("Sprite value 1 is not a number");
        }
        ...
        if (sol::type::number != item2.get_type()) {
            throw Exceptions::EstException("%s:Sprite value 2 is not a number", key.c_str());
        }
        ...
        if (sol::type::number != item3.get_type()) {
            throw Exceptions::EstException("%s:Sprite value 3 is not a number", key.c_str());
        }
        sprite_value.Y = item3;

        auto item4 = key_array[4];
        if (sol::type::number != item4.get_type()) {
            throw Exceptions::EstException("%s:Sprite value 4 is not a number", key.c_str());
        }
        sprite_value.AnchorPointX = item4;

        auto item5 = key_array[5];
        if (sol::type::number != item5.get_type()) {
            throw Exceptions::EstException("%s:Sprite value 5 is not a number", key.c_str());
        }
        sprite_value.AnchorPointY = item5;

        auto item6 = key_array[6];
        if (sol::type::number != item6.get_type()) {
            throw Exceptions::EstException("%s:Sprite value 6 is not a number", key.c_str());
        }
