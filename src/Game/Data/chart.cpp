...
Chart::Chart(Osu::Beatmap &beatmap)
{
    ...
    Logs::Puts("[Chart] Now loading osu!mania: %d", beatmap.BeatmapID);
...
Chart::Chart(BMS::BMSFile &file)
{
    ...
    Logs::Puts("[Chart] Now loading BMS: %s", file.Title);
...
Chart::Chart(O2::OJN &file, int diffIndex)
{
    ...
    Logs::Puts("[Chart] Now Loading O2Jam: %d, difficulty: %d", file.Header.songid, diffIndex);
